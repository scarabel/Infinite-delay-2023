...
The eigenvalue problem for the infinite-dimensional operator $\mathcal{A}_0+\mathcal{L}$ is turned into the eigenvalue problem for the finite-dimensional operator $\mathcal{A}_{N,0}+\mathcal{L}_N.$ The key question is how accurate are these approximations. To this aim, we carry out the same program in \cite{BMV14}, which is based on the convergence of the collocation solution of \eqref{eigIVP}. For this reason we focus on this fundamental result, while we simply recall the derivation of the characteristic equation for $\mathcal{A}_{N,0}+\mathcal{L}_N$ and the final convergence theorem. 
Indeed we study the collocation approximation for a more general problem, which includes as particular cases, not only \eqref{eigIVP} but also the equation $(\mathcal{A}_0-\lambda)x=z,$ whose solution allows to define the resolvent operator of $\mathcal{A}_0,$ i.e. $(\mathcal{A}_0-\lambda)^{-1}.$

Let $Z \subseteq X$ be a Banach space of (equivalences classes of) functions which admit one (and only one) continuous representative, equipped with the $\|\cdot \|_X$ norm. Given $\lambda \in \mathbb{C},$ with $\Re \lambda  > -\rho_1$, $\beta \in \mathbb{C}^{\overline{d}}$ and $z \in Z,$ we denote $x(\cdot;\lambda,\beta,z) \in Z$ the solution of the following initial value problem
\begin{equation}\label{IVPZ}
\left\{\ba{l}
x'=(\lambda +\rho)x+z, \  \theta \in \mathbb{R}_-,\\
x(\theta_{0})=\beta,
\ea
\right.
\end{equation}
We have that $x=e(\lambda+\rho)\beta+\mathcal{H}(\lambda +\rho)z=w(e(\lambda)\beta+\mathcal{H}(\lambda)w^{-1}z),$  and, when $\beta$ is the solution of $\beta=\mathcal{B}(e(\lambda+\rho)\beta+\mathcal{H}(\lambda +\rho)z)$, we have that $x=(\mathcal{A}_0-\lambda I_X)^{-1}z.$ So the equations \eqref{IVPZ} allows to represent the resolvent equation of $\mathcal{A}_0$ in $Z,$ and the next theorem furnishes the estimate of the collocation error. 

\begin{theorem}\label{Th00}
Let $\rho=\rho_1+\epsilon,\ \epsilon>0$ and $B$ an open ball in $\mathbb{C}_{\rho_1}$. If
\bi
\item[\texttt{(H1)}] $\| (\mathcal{I}_{N}-I_{X})\mathcal{H}(0) \|_{Z \to Z} \rightarrow 0 \  \text {as } N \rightarrow\infty,$
\ei
then there exists $\overline{N}:=\overline{N}(B)$ such that, for $N \geq \overline{N}$, $\lambda \in B$, $\beta \in \mathbb{C}^{\overline{d}}$ and $z \in Z$, the solution $x_{N,0}(\cdot;\lambda,\beta,z) \in w\Pi_N^{\overline{d}}$ of the collocation equations at the nodes $\Theta_N,$ i.e.
\begin{equation}\label{collZ}
\left\{\ba{l}
x_{N,0}'(\theta_{i})=(\lambda +\rho)x_{N,0}(\theta_{i})+z(\theta_{i}), \ i=1,\dots,N,\\
x_{N,0}(\theta_{0})=\beta.
\ea
\right.
\end{equation} exists and it is unique. Moreover, if 
\bi
\item[\texttt{(H2)}] $\|(\mathcal{I}_{N}-I_{X})z \|_X \rightarrow 0 \  \text {as } N \rightarrow\infty, \ \forall z \in Z, $
\ei
then, for $N \geq \overline{N},$ there exists a positive constant $\overline{C}(B),$ independent of $N,$ such that 
\be\label{Ebound1Z}
\|x_{N,0}(\cdot;\lambda,\beta,z)-x(\cdot;\lambda,\beta,z)\|_X \leq \overline{C}(B) R_N(B) \max \left(|\beta|,\|z\|_X\right )
\end{equation}
\be\label{Ebound2Z}
\|x'_{N,0}(\cdot;\lambda,\beta,z)-x'(\cdot;\lambda,\beta,z)\|_X \leq \overline{C}(B)  R_N(B) \max \left(|\beta|,\|z\|_X\right ),
\end{equation}
and
\be\label{RNZ}
R_N(B):=\sup_{\footnotesize{\ba{c}\lambda \in \overline{B}, \\\beta \in \mathbb{C}^{\overline{d}} \neq 0, z \in Z \neq 0\ea} }\frac{\| r_N(\cdot;\lambda,\beta,z) \|_{X}}{\max \left(|\beta|,\|z\|_X\right )} \end{equation} with $r_N(\theta;\lambda,\beta,z):=(\mathcal{I}_{N}-I_{X})x'(\theta;\lambda,\beta,z), \ \theta \in \mathbb{R}_{-},$  and the convergence follows.
\end{theorem}

\begin{proof}
The proof follows the same lines as previous proofs of related results in \cite{BMV14,DSV20}, but we reproduce it for the sake of completeness. For ease of notation, we denote $\mu:=\lambda+\rho,$ and we omit to write the dependence on the parameters explicitly. By subtracting (\ref{IVPZ}) from the collocation equation \eqref{collZ}, and setting $e_{N}:=x_{N,0}-x$ and $r_{N}:=(\mathcal{I}_{N}-I_X)x',$ we get the following equation 
\begin{equation}\label{EEQ1}
e'_{N}=\mu \mathcal{I}_{N}e_{N}+r_{N}.
\end{equation}
Let $W_0:=\{ x \in  Z \ | \ x' \in Z \text{ and } x(0)=0 \}.$ Then the solution of (\ref{EEQ1}) is the function $e_{N}=V e'_{N} \in W_0,$ with $e'_{N} \in Z$ solution of
\begin{equation}\label{EEQ2}
e'_{N}-\mu \mathcal{I}_{N}\mathcal{H}(0)e'_{N}=r_{N},
\end{equation}
and viceversa.

From \eqref{Hbound} we have that the operator $I_X-\mu \mathcal{H}(0)$ is invertible in $Z$, and it is bounded, i.e.
$$
\| (I_X-\mu \mathcal{H}(0))^{-1} \|_{Z \to Z} \leq C(B),
$$
with $C(B):=\sup_{\lambda \in \overline{B}} (1+ |\lambda+\rho|C_0(\lambda+\rho)).$ From (H1) and from the Banach perturbation Lemma, we conclude that there exists a positive integer $\overline{N}:=\overline{N}(B)$ such that $I_X-\mu\mathcal{I}_{N}\mathcal{H}(0)$ is invertible in $Z$ for $N\geq \overline{N},$ and
\begin{displaymath}
\left\| (I_X-\mu \mathcal{I}_{N}\mathcal{H}(0))^{-1}\right\|_{Z \to Z}\leq 2\left\|(I_X-\mu \mathcal{H}(0))^{-1}\right\|_{Z \to Z} \leq 2 C(B).
\end{displaymath}
Hence, for $N\geq \overline{N},$ $e'_{N}=(I_X-\mu \mathcal{I}_{N}\mathcal{H}(0))^{-1} r_N$ is the unique solution of (\ref{EEQ2}) in $Z$ and $e_{N}=\mathcal{H}(0)(I_X-\mu\mathcal{I}_{N}\mathcal{H}(0))^{-1}{r}_{N}=(I-\mu \mathcal{H}(0) \mathcal{I}_{N})^{-1}\mathcal{H}(0){r}_{N}$ is the unique solution of \eqref{EEQ1} in $W_0$. Thus there exits a unique $x_{N,0} \in w\Pi_N^{\overline{d}}$ and 
\begin{displaymath}
\ba{l}
\|e'_{N}\|_X  \leq 2 C(B)\|r_{N}\|_X,\\[0.4cm]
\|e_{N}\|_X \leq \|\mathcal{H}(0)e'_{N}\|_X \leq 2 \| \mathcal{H}(0)\|_{X \to X} C(B)\|r_{N}\|_X.\\
\ea
\end{displaymath}
Then the bounds \eqref{Ebound1}-\eqref{Ebound2} hold with $\overline{C}(B):= 2C(B)\max\{1,\frac{1}{\rho} \}$. Finally, since $x' \in Z$, from (H2), we have that $\|r_{N}\|_x=\|(\mathcal{I}_{N}-I_X)x' \|_X \to 0$ as $N \to \infty.$ and the convergence follows for all $\lambda \in B$, $\beta \in \mathbb{C}^{\overline{d}}$ and $z \in Z.$
\end{proof}

Let's go back to \eqref{eigIVP} and we consider $ z \in \Xi_d$. From Theorem \ref{Th00} we get the following convergence result for  the solution $x_{N,0}(\cdot;\lambda,\beta,\gamma) \in w\Pi_N^{\overline{d}}$ of the collocation equations at the nodes $\Theta_N$, i.e. 
...
\begin{corollary}\label{Th0}
Let $\rho=\rho_1+\epsilon,\ \epsilon>0$ and $B$ an open ball in $\mathbb{C}_{\rho_1}$. If $H(1)$ holds, then there exists $\overline{N}:=\overline{N}(B)$ such that, for $N \geq \overline{N}$, the collocation solution $x_{N,0}$ of \eqref{coll} exists and it is unique, for $\lambda \in B$, $\beta \in \mathbb{C}^{\overline{d}}$ and $\gamma \in \mathbb{C}^{d}.$  Moreover, under assumption $H(2)$, there exists a positive constant $\overline{C}(B)$ independent of $N$ such that, for  $N \geq \overline{N},$
\be\label{Ebound1}
\|x_{N,0}(\cdot;\overline{\lambda},\beta,\gamma)-x(\cdot;\overline{\lambda},\beta,\gamma)\|_X \leq \overline{C}(B) R_N(B) |(\beta;\gamma)|,
\end{equation}
\be\label{Ebound2}
\|x'_{N,0}(\cdot;\lambda,\beta,\gamma)-x'(\cdot;\lambda,\beta,\gamma)\|_X \leq \overline{C}(B)  R_N(B) |(\beta;\gamma)|,
\end{equation} where
\be\label{RN}
R_N(B):=\sup_{\lambda \in \overline{B}, (\beta;\gamma) \in \mathbb{C}^{\overline{d}+d} \setminus \{ 0\} } \frac{\| r_N(\cdot;\lambda,\beta,\gamma) \|_X}{|(\beta;\gamma)|} \end{equation} with $r_N(\theta;\lambda,\beta,\gamma):=(\mathcal{I}_{N}-I_X)x'(\theta;\lambda,\beta,\gamma), \ \theta \in \mathbb{C}_{-}.$  The convergence of the collocation solution \eqref{coll} to $x(\cdot;\lambda,\beta,\gamma)$ follows for all $\lambda \in B,$ $\beta \in \mathbb{C}^{\overline{d}}$ and $\gamma \in \mathbb{C}^{d},$ and the convergence follows.
\end{corollary}

\begin{remark}\label{r1}
For the abstract delay equations , we have that $(\mathcal{I}_{N}-I_X)\xi_i=0, \ i=0,\ldots,d$, and so $r_N(\theta;\lambda,\beta,\gamma)=(\lambda +\rho)(\mathcal{I}_{N}-I_X)x(\theta;\lambda,\beta,\gamma)$. This is not true for the truncated interpolation scheme. 
\end{remark}

Now we are ready to derive the discrete versione of characteristic equation, i.e. the characteristic equation for $\mathcal{A}_{N,0} +\mathcal{L}_N$ \cite[Proposition 5.2]{BMV14}.

\begin{proposition}\label{PCEN}
The eigenvalues of the finite-dimensional operator $\mathcal{A}_{0,N} +\mathcal{L}_N$ on $\mathbb{C}_\rho$ are the roots of the characteristic equation
\begin{equation}\label{CEN}
\text{det}(I_{\overline{d}+d}-\mathcal{A}_N(\lambda))=0,
\end{equation}
where $\mathcal{A}_N(\lambda): \mathbb{C}^{\overline{d}+d} \to \mathbb{C}^{\overline{d}+d}$ is the linear operator
...
By applying the Rouch\'e's Theorem to compare the roots of the continuous characteristic equation \eqref{CE} and the discrete one \eqref{CEN}, we obtain the following convergence result \cite[Section 5.3.2]{BMV14}.

\begin{theorem}\label{Th2}
Let $\rho=\rho_1+\epsilon,\ \epsilon>0,$ and $\lambda \in \mathbb{C}, \ \Re\lambda> -\rho_1$ a root of the equation \eqref{CE} with multiplicity $m$. Let $B$ be an open ball in $\mathbb{C}_{\rho_1}$ of center $\lambda$ such that $\lambda$ is the unique root of \eqref{CE} in $B$. Under the assumptions of Theorem \ref{Th0}, there exits $N_1:=N_1(B),$ such that for $N\geq N_1,$ the equation \eqref{CEN} has $m$ roots $\lambda_{i,N}, i=1,\dots,m$ (taking into account multiplicities) and 
\bd
\max_{i=1,\dots,m}| \lambda-\lambda_{i,N}| \leq \left(C_1(B) R_N(B)\right)^{1/m},
\ed
with $C_1(B)$ a positive constant independent of $N$.
\end{theorem}
